...
import { ChefHat, Search, Loader2, CheckCircle2, AlertCircle, RefreshCw, X, UtensilsCrossed, Pencil } from 'lucide-react';
...
export const KitchenStockTab: React.FC = () => {
  ...
  const [stockOutModalItem, setStockOutModalItem] = useState<FlatKitchenItem | null>(null);

  // Stock In Modal state
  const [stockInModalItem, setStockInModalItem] = useState<FlatKitchenItem | null>(null);
  const [stockInQuantity, setStockInQuantity] = useState<number>(50);

  // Compact Stock Edit Modal state
  const [editStockModalItem, setEditStockModalItem] = useState<FlatKitchenItem | null>(null);
        ...
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted mt-0.5">
              Control kitchen inventory and availability in real time with preserved stock tracking
            </p>
          </div>
        </div>
