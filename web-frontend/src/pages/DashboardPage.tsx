...
export const DashboardPage: React.FC<DashboardPageProps> = ({ onNavigate }) => {
  ...
  // Demo mode toggles to true automatically if active sessions database is empty
  const [useDemoData, setUseDemoData] = useState(tokens.length === 0);
  ...
  const handleExtendSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (useDemoData || extendingToken.id.startsWith('mock-')) {
        await new Promise(resolve => setTimeout(resolve, 500));
        showToast(`Session for ${extendingToken.tokenNumber} extended by ${extraMinutes} mins. (Demo Mode)`, 'success');
  ...
  const handleCloseSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (useDemoData || closingToken.id.startsWith('mock-')) {
        await new Promise(resolve => setTimeout(resolve, 500));
        showToast(`Session ${closingToken.tokenNumber} closed (${closeReason}). (Demo Mode)`, 'success');
  ...
  const activeTokens = useDemoData ? MOCK_TOKENS : tokens;
  const activeTokensCount = useDemoData ? MOCK_KPIs.activeSessions : tokens.length;
  const occupiedTablesCount = useDemoData ? MOCK_KPIs.occupiedTables : tables.filter(t => t.status === 'occupied').length;
  const displayTablesCount = tables.length > 0 ? tables.length : MOCK_KPIs.totalTables;
  const totalCapacity = tables.length > 0 ? tables.reduce((acc, t) => acc + t.capacity, 0) : MOCK_KPIs.totalCapacity;
  const totalGuestsInHouse = useDemoData ? MOCK_KPIs.inHouseGuests : tokens.reduce((acc, tk) => acc + tk.personsCount, 0);
  const totalRedemptionsUsed = useDemoData ? MOCK_KPIs.drinksRedeemed : tokens.reduce((acc, tk) => acc + tk.redemptionsUsed, 0);
  const totalRevenue = useDemoData ? MOCK_KPIs.revenue : tokens.reduce((acc, tk) => acc + (tk.amountPaid || 0), 0);
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 p-4 sm:p-6 glass-panel rounded-2xl border border-border-main">
        ...
        <div className="flex items-center gap-3 self-end md:self-auto">
          ...
          <button
            ...
            className={`w-12 h-6 rounded-full transition-all relative flex items-center p-1 cursor-pointer ${useDemoData ? 'bg-primary justify-end' : 'bg-bg-primary border border-border-main justify-start'}`}
          >
            ...
            <div className="relative pl-6 space-y-4 border-l border-border-main ml-3 py-1">
              {MOCK_ACTIVITIES.map((act) => (
                <div key={act.id} className="relative text-xs">
                  <div className="absolute -left-[30px] top-1 w-2.5 h-2.5 rounded-full bg-[#8D6CE5] border-2 border-bg-surface shadow-[0_0_8px_rgba(141,108,229,0.6)] z-10 animate-pulse" />
