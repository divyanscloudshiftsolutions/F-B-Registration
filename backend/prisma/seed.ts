...
async function main() {
  ...
  // 8. Seed Customers and Customer Sessions (Mock data)
  console.log('Seeding customer sessions mock data...');

  const adminUser = await prisma.user.findFirst({ where: { username: 'admin' } });
  const bartenderUser = await prisma.user.findFirst({ where: { username: 'bartender' } });
  const receptionistUser = await prisma.user.findFirst({ where: { username: 'receptionist' } });

  const adminId = adminUser?.id || '';
  const bartenderId = bartenderUser?.id || '';
  const receptionistId = receptionistUser?.id || '';

  const standingBarId = dbPlaceTypes['STANDING_BAR'];
  const premiumLoungeId = dbPlaceTypes['PREMIUM_LOUNGE'];

  // Retrieve table ids
  const tableS01 = await prisma.table.findFirst({ where: { tableNumber: 'S-01', placeTypeId: standingBarId } });
  const tableS02 = await prisma.table.findFirst({ where: { tableNumber: 'S-02', placeTypeId: standingBarId } });
  const tableS03 = await prisma.table.findFirst({ where: { tableNumber: 'S-03', placeTypeId: standingBarId } });
  const tableL01 = await prisma.table.findFirst({ where: { tableNumber: 'L-01', placeTypeId: premiumLoungeId } });
  const tableL02 = await prisma.table.findFirst({ where: { tableNumber: 'L-02', placeTypeId: premiumLoungeId } });
  ...
  // 1. Rahul Sharma - ACTIVE on S-01
  const token1 = await prisma.token.create({
    data: {
      tokenNumber: `BAR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-00001`,
      customerId: customer1.id,
      personsCount: 2,
      placeTypeId: standingBarId,
      tableId: tableS01?.id,
      amountPaid: 1000.0,
      paymentVerified: true,
      startTime: new Date(now.getTime() - 45 * 60000), // 45 mins ago
      endTime: new Date(now.getTime() + 75 * 60000), // 75 mins left
  ...
  if (tableS01) {
    await prisma.table.update({
      ...
      data: {
        ...
        occupiedSince: token1.startTime,
      }
    });
  }
  ...
  await prisma.redemption.create({
    data: {
      ...
      redeemedAt: new Date(now.getTime() - 30 * 60000),
      bartenderId: bartenderId,
      notes: 'Kingfisher Premium Beer',
    }
  });

  // 2. Priya Patel - EXTENDED on L-01
  const token2 = await prisma.token.create({
    data: {
      tokenNumber: `BAR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-00002`,
      customerId: customer2.id,
      personsCount: 4,
      placeTypeId: premiumLoungeId,
      tableId: tableL01?.id,
      amountPaid: 6000.0,
      paymentVerified: true,
      startTime: new Date(now.getTime() - 150 * 60000), // 2.5 hours ago
      endTime: new Date(now.getTime() + 90 * 60000), // 1.5 hours left
      totalRedemptionsAllowed: 16,
      redemptionsUsed: 5,
      status: 'EXTENDED',
      issuedBy: receptionistId,
      deliveryMode: 'NFC_CARD',
  ...
  if (tableL01) {
    await prisma.table.update({
      ...
      data: {
        ...
        occupiedSince: token2.startTime,
      }
    });
  }
  await prisma.card.update({
  ...
  for (let seq = 1; seq <= 5; seq++) {
    await prisma.redemption.create({
      data: {
        ...
        redeemedAt: new Date(now.getTime() - (150 - seq * 20) * 60000),
        bartenderId: bartenderId,
        notes: seq % 2 === 0 ? 'LIIT Cocktail' : 'Glenfiddich 12 Year Whisky',
      }
    });
  }

  // Create extension record
  await prisma.tokenExtension.create({
    data: {
      tokenId: token2.id,
      extraMinutes: 60,
      additionalAmount: 1200.0,
      approvedBy: adminId,
      extendedAt: new Date(now.getTime() - 40 * 60000),
      newEndTime: token2.endTime,
    }
  });

  // 3. Amit Verma - PENDING_PAYMENT
  await prisma.token.create({
    data: {
      tokenNumber: `BAR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-00003`,
      ...
      startTime: new Date(now.getTime() - 10 * 60000), // 10 mins ago
      ...
      emailSent: true,
      emailSentAt: new Date(now.getTime() - 10 * 60000),
      emailDeliveryStatus: 'SENT',
    }
  });

  // 4. Sneha Reddy - CLOSED (Checked out)
  const token4 = await prisma.token.create({
    data: {
      tokenNumber: `BAR-${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}-00004`,
      ...
      amountPaid: 1000.0,
      paymentVerified: true,
      startTime: new Date(now.getTime() - 240 * 60000), // 4 hours ago
      endTime: new Date(now.getTime() - 120 * 60000), // 2 hours ago
      totalRedemptionsAllowed: 4,
      redemptionsUsed: 3,
      status: 'CLOSED',
      issuedBy: receptionistId,
      closedBy: receptionistId,
      closedAt: new Date(now.getTime() - 130 * 60000),
      closeReason: 'CHECKOUT',
      deliveryMode: 'EMAIL_QR',
      emailSent: true,
      emailSentAt: new Date(now.getTime() - 240 * 60000),
      emailDeliveryStatus: 'SENT',
    }
  });

  // Redemptions for Sneha Reddy
  for (let seq = 1; seq <= 3; seq++) {
    await prisma.redemption.create({
      data: {
        tokenId: token4.id,
        redemptionSequence: seq,
        redeemedAt: new Date(now.getTime() - (240 - seq * 30) * 60000),
        bartenderId: bartenderId,
        notes: 'Budweiser Premium Pint',
      }
    });
  }

  // Create log entries for occupied S-01, L-01 and closed S-03
  await prisma.tableOccupancyLog.createMany({
    data: [
      {
        id: '11111111-1111-4111-a111-111111111111',
        tableId: tableS01?.id || '',
        tokenId: token1.id,
        occupiedAt: token1.startTime,
      },
      {
        id: '22222222-2222-4222-a222-222222222222',
        tableId: tableL01?.id || '',
        tokenId: token2.id,
        occupiedAt: token2.startTime,
      },
      {
        id: '33333333-3333-4333-a333-333333333333',
        tableId: tableS03?.id || '',
        tokenId: token4.id,
        occupiedAt: token4.startTime,
        vacatedAt: token4.closedAt,
        durationMinutes: 110,
      }
    ]
  });

  console.log('Customer sessions and mock operational data seeded.');
