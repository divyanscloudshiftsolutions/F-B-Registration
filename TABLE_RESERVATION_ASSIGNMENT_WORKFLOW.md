...
### Test Case 14: Strict Reservation Independence, Separate Process Identity & Non-Destructive Check-In

#### A. Objective
Verify that an active Reservation exists as an independent business process with its own identity (`reservationId`). Starting a check-in, assigning another table, entering duplicate phone/email, performing validation, abandoning a check-in, stopping a check-in, navigating, or refreshing the page must NEVER cancel, delete, release, or silently convert another user's reservation.

#### B. Key Rules Enforced:
1. **Dedicated Lifecycle**: A reservation remains in `PENDING` status until:
   - The reservation owner explicitly cancels it from the Reservations page (`POST /reservations/:id/cancel`).
   - An authorized Administrator explicitly cancels/releases it from the Administrator Table Management panel (`POST /reservations/:id/cancel`).
2. **Process Identity Separation**: A Check-In session (`tokenId` / `tokenNumber`) and a Reservation (`reservationId`) are completely distinct entities. Matching phone or email addresses are conflict validation attributes only—never process identities.
3. **Non-Destructive Validation**: Duplicate phone/email validation is 100% read-only. Detecting a conflict prevents the conflicting action and identifies the reservation owner without mutating the reservation record.
4. **Non-Destructive Check-In Stop/Abandon**: Calling `POST /api/check-in/stop` or abandoning a check-in draft terminates only the in-progress check-in token and unlocks table locks. If the table has an active `PENDING` reservation, the table status strictly reverts to `reserved` and the reservation remains untouched.
5. **Strict Ownership Access**: Other receptionists/staff (User B) can view User A's reserved table and reservation details, but cannot check in, assign, cancel, or release User A's reservation. Check-In and Cancel buttons are locked with descriptive ownership tooltips and protected by backend authorization (`403 RESERVATION_NOT_OWNED`).

---

### Test Case 15: First-Claim Priority, Table Concurrency & Four-Way Concurrency Matrix

#### A. Objective
Verify that when two users simultaneously attempt to claim or assign the same table, the operation resolves atomically via PostgreSQL row locking (`SELECT ... FOR UPDATE`) and Redis key acquisition (`table:lock:${tableId}`). The first claimant receives `200 OK` and ownership; the second claimant is rejected with `409 Conflict (TABLE_STATUS_INVALID)` and see the table locked by the winning user. Furthermore, verify bidirectional conflict resolution across all four quadrants of the concurrency matrix (Check-In ↔ Check-In, Check-In ↔ Reservation, Reservation ↔ Check-In, Reservation ↔ Reservation).

#### B. Four-Way Concurrency Matrix:
| Existing Process | New Process Attempt | Status Conflict | Resolution |
| :--- | :--- | :--- | :--- |
| **Check-In** | **Check-In** | Phone / Email match | `409 Conflict` (identifies active check-in user) |
| **Check-In** | **Reservation** | Phone / Email match | `409 Conflict: PHONE_CONFLICT / EMAIL_CONFLICT` |
| **Reservation** | **Check-In** | Phone / Email / Table match | `409 Conflict` (identifies reservation owner; blocks non-owner) |
| **Reservation** | **Reservation** | Phone / Email / Table match | `409 Conflict: PHONE_CONFLICT / EMAIL_CONFLICT / TABLE_NOT_AVAILABLE` |

#### C. Verification Guarantees:
1. **Authoritative Backend Locks**: The backend enforces first-claim priority at the database row-level (`FOR UPDATE`). Stale frontend state cannot bypass backend validation.
2. **Lock Reacquisition Isolation**: A locked table (`in_checkin`) can only be re-locked or continued by the specific user holding the lock (`lockedBy === userId` or `lockedByUserId === userId`). Different users are strictly prevented from re-acquiring or overriding an in-progress check-in lock.
3. **Pre-Payment & Token Creation Integrity**: Pre-payment validation (`POST /check-in/pre-payment-validate`) and token creation (`createToken`) strictly reject occupied tables and tables reserved by other users (`409 Conflict` / `403 Forbidden`).
4. **Clean Release with Preserved State**: Releasing or unlocking tables returns them to `reserved` if a pending reservation is attached, broadcasting complete owner metadata over WebSockets.

---

### Test Case 16: Authoritative Resource Reusability & Zero Stale Conflicts Upon Reservation Cancellation

#### A. Objective
Verify that once an active Reservation is explicitly cancelled (`POST /api/reservations/:id/cancel`), all associated customer details (Phone Number and Email) and physical table resources are immediately and authoritatively released across PostgreSQL, Redis, backend duplicate validation queries, frontend DataContext state, and multi-user views. A fresh Check-In or Reservation using the same phone/email must be allowed immediately without remaining blocked by stale conflict cache or historical records.

```mermaid
sequenceDiagram
    autonumber
    actor Receptionist as User A (Receptionist)
    actor Admin as User B (Admin)
    participant Backend as Backend API & PostgreSQL
    participant Redis as Redis Cache
    participant WebSocket as WebSocket Broadcast

    Note over Receptionist,Admin: Active Reservation Phase
    Receptionist->>Backend: Create Reservation (Phone: 9876543210, Email: vip@gmail.com)
    Backend-->>Receptionist: 201 Created (Status: PENDING)
    Admin->>Backend: Validate Duplicate (Phone: 9876543210)
    Backend-->>Admin: 200 OK (Conflict: true, Owner: Receptionist)

    Note over Receptionist,Admin: Explicit Cancellation Phase
    Receptionist->>Backend: Cancel Reservation (POST /reservations/:id/cancel)
    Backend->>Backend: Update status = 'CANCELLED', Release Table to available
    Backend->>Redis: Invalidate table:lock, checkin:active:phone/email keys
    Backend->>WebSocket: Broadcast table:updated (status: 'available')
    Backend-->>Receptionist: 200 OK (Reservation cancelled successfully)

    Note over Receptionist,Admin: Post-Cancellation Reusability Phase
    Admin->>Backend: Validate Duplicate / Fresh Check-In (Phone: 9876543210)
    Backend-->>Admin: 200 OK (Conflict: false, Phone/Email Reusable)
    Admin->>Backend: Proceed with Check-In
    Backend-->>Admin: Check-In Allowed
```

#### B. Architectural Rules & Validation Lifecycle:
1. **Authoritative Backend Query Filtering**:
   - `POST /api/check-in/validate-duplicate` queries `Reservation` records strictly with `where: { status: 'PENDING' }`.
   - Historical or inactive reservations (`CANCELLED`, `ASSIGNED`, `EXPIRED`, `COMPLETED`) are completely excluded from participating in conflict detection.
2. **Atomic Invalidation in PostgreSQL & Redis**:
   - Explicit cancellation (`POST /api/reservations/:id/cancel`) marks the reservation `status = 'CANCELLED'`.
   - Purges Redis table locks (`table:lock:${tableId}`), table status caches (`table:${tableId}:status`), and any active checkin phone/email keys (`checkin:active:phone:${phone}`, `checkin:active:email:${email}`).
3. **Reactive Frontend Synchronization**:
   - Both `CheckInPage.tsx` and `TablesPage.tsx` listen to the `app:global-refresh` custom event (emitted upon clicking the header refresh button) and reactively observe changes to `[reservations, activeTokens]`.
